...
let
  ...
  patchSystemUIGoogle = ''
    substituteInPlace device${
      lib.optionalString (config.deviceFamily == "redfin") "-common"
    }.mk --replace SystemUIGoogle SystemUI
    substituteInPlace overlay/frameworks/base/core/res/res/values/config.xml --replace SystemUIGoogle SystemUI
  '';
in
mkIf (config.flavor == "vanilla") (mkMerge [

  ### Generic stuff ###
  {
    source.dirs = lib.importJSON (
      ./. + "/${builtins.toString config.androidVersion}/repo-${config.source.manifest.rev}.json"
    );
    ...
    warnings =
      ...
      ) "Selected older version of android. Security updates may be out-of-date";
  ...
  (mkIf
    ...
    {
      source.dirs."packages/apps/Launcher3".patches = [
        (./. + "/${toString config.androidVersion}/disable-quicksearch.patch")
      ];
    }
  )
  ...
  (mkIf
    ...
    )
    {
      ...
      source.dirs."device/google/marlin".postPatch =
        "substituteInPlace common/base.mk --replace SystemUIGoogle SystemUI";
    }
  )
  (mkIf
    (
  ...
  (mkIf
    ...
    (
      let
        dirName =
          if config.deviceFamily == "redfin" then
            "redbull"
          else if config.deviceFamily == "raviole" then
            "gs101"
          else
            config.deviceFamily;
      in
