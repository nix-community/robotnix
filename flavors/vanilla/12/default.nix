...
(mkIf (config.flavor == "vanilla" && config.androidVersion == 12) (mkMerge [
  {
    ...
    source.manifest.rev = mkDefault (
      if (config.deviceFamily == "raviole") then "android-12.0.0_r32" else "android-12.0.0_r28"
    );
    apv.buildID = mkDefault (
      if (config.deviceFamily == "raviole") then "SQ1D.220205.004" else "SQ1A.220205.002"
    );
  ...
  (mkIf (config.deviceFamily == "crosshatch") {
    warnings = [
      "crosshatch and blueline are no longer receiving monthly vendor security updates from Google"
    ];
      ...
      "robotnix/prebuilt/com.google.pixel.camera.hal" = {
        src =
          let
            ...
            '';
          in
          pkgs.runCommand "com.google.pixel.camera.hal" { } ''
            mkdir -p $out
