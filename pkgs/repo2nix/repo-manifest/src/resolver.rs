...
pub fn merge_manifests(
    manifest: &mut xml::Manifest,
    submanifest: &xml::Manifest,
) -> Result<(), RecursivelyReadManifestFilesError> {
    ...
    for remote in submanifest.remotes.iter() {
        if manifest.remotes.iter().any(|r| r.name == remote.name) {
            return Err(RecursivelyReadManifestFilesError::DuplicateRemote(
    ...
    for project in submanifest.projects.iter() {
        if manifest
            .projects
            .iter()
            .any(|p| p.path.as_ref().unwrap() == project.path.as_ref().unwrap())
        {
            return Err(RecursivelyReadManifestFilesError::DuplicatePath(
                project.path.as_ref().unwrap().to_path_buf(),
            ));
        }
...
pub async fn recursively_read_manifest_files(
    root_path: &Path,
    manifest_file: &Path,
) -> Result<xml::Manifest, RecursivelyReadManifestFilesError> {
    let mut manifest = read_manifest_file(&root_path.join(manifest_file))
        .await
        .map_err(
            |e| RecursivelyReadManifestFilesError::ManifestReadFileError {
...
pub fn resolve_manifest(
    manifest_xml: &xml::Manifest,
    base_url: &Url,
) -> Result<Manifest, ResolveManifestError> {
    ...
    for remote_xml in manifest_xml.remotes.iter() {
        let url = match Url::parse(&remote_xml.fetch) {
            ...
            Err(ParseError::RelativeUrlWithoutBase) => {
                ...
                let path = base_path
                    .parent()
                    .ok_or(ResolveManifestError::InvalidRelativeRemoteURL(
                        remote_xml.fetch.clone(),
                    ))?
                    .join(&remote_xml.fetch);
                let mut url = base_url.clone();
                url.set_path(
                    path.to_str()
                        .ok_or(ResolveManifestError::InvalidUTF8(path.clone()))?,
                );
    ...
    if let Some(default_remote_xml) = &manifest_xml.default {
        let remote = manifest.remotes.get(&default_remote_xml.remote).ok_or(
            ResolveManifestError::DefaultRemoteNotFound(default_remote_xml.remote.clone()),
        )?;
        manifest.default_remote = Some(Remote {
            ...
            revision: remote
                .revision
                .clone()
                .or(default_remote_xml.revision.clone()),
        });
    }
    ...
    for project_xml in manifest_xml.projects.iter() {
        ...
        let remote =
            match &project_xml.remote {
                Some(remote_name) => manifest.remotes.get(remote_name).ok_or(
                    ResolveManifestError::RemoteNotFound(name.clone(), remote_name.clone()),
                )?,
                None => manifest
                    .default_remote
                    .as_ref()
                    .ok_or(ResolveManifestError::MissingRemote(name.clone()))?,
            };
        let path = project_xml
            .path
            .clone()
            .unwrap_or(PathBuf::from(&project_xml.name));
        let project = Project {
            path: path,
            groups: project_xml
                .groups
                ...
                .unwrap_or(vec![]),
            linkfiles: project_xml
                .linkfiles
                .iter()
                .cloned()
                .map(LinkCopyFile::from_xml)
                .collect(),
            copyfiles: project_xml
                .copyfiles
                .iter()
                .cloned()
                .map(LinkCopyFile::from_xml)
