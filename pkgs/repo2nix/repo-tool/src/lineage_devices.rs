...
pub async fn fetch_hudson_devices()
-> Result<HashMap<String, HudsonDeviceInfo>, FetchHudsonDevicesError> {
    ...
    let text = str::from_utf8(&bytes).map_err(FetchHudsonDevicesError::Utf8)?;
...
pub async fn get_device_repos() -> Result<Vec<(String, String)>, GetDeviceReposError> {
    ...
    let mirror_manifest = read_manifest_file(&mirror_fetch.path.join("default.xml")).await?;
...
pub async fn get_devices(
...
) -> Result<BTreeMap<String, DeviceInfo>, GetDevicesError> {
    ...
    for name in hudson_keys.iter() {
        if allowlist.as_ref().map(|x| x.contains(name)).unwrap_or(true)
            && blocklist
                .as_ref()
                ...
                .unwrap_or(true)
        {
            ...
            let possible_vendors: Vec<_> = device_repos
                .iter()
                .filter(|x| x.1 == *name)
                .map(|x| x.0.clone())
                .collect();
            ...
            for vendor in possible_vendors {
                let branches =
                    get_repo_branches(&format!("LineageOS/android_device_{vendor}_{name}"))
                        .await
                        .map_err(GetDevicesError::RepoBranches)?;

                if branches
                    .iter()
                    .any(|x| *x == hudson_to_device_repo_branch(&hudson_data.branch))
                {
                    ...
                    for branch in branches {
                        branch_repos.insert(
                            branch.clone(),
                            GitRepoRef {
                                repo_url: Url::parse(&format!(
                                    "https://github.com/LineageOS/android_device_{vendor}_{name}"
                                ))
                                .map_err(GetDevicesError::Url)?,
                                revision: format!(
                                    "refs/heads/{}",
                                    hudson_to_device_repo_branch(&branch)
                                ),
                                fetch_lfs: true,
