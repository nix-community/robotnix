...
with lib;
{
  ...
  config = mkIf config.source.overridePrebuilts {
    source.dirs."build/soong".patches = [ ../patches/disable-fail-warnings.patch ];
    ...
    #source.dirs."prebuilts/go/linux-x86".contents = "${pkgs.go_1_12}/share/go"; # Seems to work
    source.dirs."prebuilts/python/linux-x86/2.7.5".contents = pkgs.python27.withPackages (p: with p;
      [ #selinux
