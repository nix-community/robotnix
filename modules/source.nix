...
in
{
    ...
    dirs = mkIf config.source.manifest.enable (
      let
        ...
        filteredEntries = lib.filterAttrs (
          ...
          && (builtins.any (
            cat: builtins.elem cat entry.project.categories
          ) config.source.manifest.categories)
        ) entries;
  ...
  config = {
    assertions = [
      {
        assertion =
          config.source.manifest.enable -> (lib.importJSON config.source.manifest.lockfile).fetch_completed;
